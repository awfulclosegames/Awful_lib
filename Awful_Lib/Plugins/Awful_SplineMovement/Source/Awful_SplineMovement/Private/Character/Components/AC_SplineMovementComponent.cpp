...
static TAutoConsoleVariable<bool> CVarAC_DebugReportSplineDecoupling(
	TEXT("Awful.SplineMovement.Decoupling.DebugReport"),
	0,
	TEXT("Debug report Spline decoupling from Character above the DebugToleranceXY.\n")
	TEXT("0: Disabled.\n")
	TEXT("1: Enabled."),
	ECVF_Scalability | ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarAC_DebugSplineDecouplingTolerance(
	TEXT("Awful.SplineMovement.Decoupling.DebugTolerance"),
	10.0f,
	TEXT("Debug value to check against if the SplineState is decoupled.\n"),
	ECVF_Scalability | ECVF_RenderThreadSafe);

static TAutoConsoleVariable<bool> CVarAC_DebugTrapSplineDecoupling(
	TEXT("Awful.SplineMovement.Decoupling.DebugTrap"),
	0,
	TEXT("Asserts as soon as SplineState pos to Character is greater than DecouplingToleranceXY.\n")
	TEXT("0: Log and DebugGfx.\n")
	TEXT("1: Trap / Assert."),
	ECVF_Scalability | ECVF_RenderThreadSafe);
#endif
...
void UAC_SplineMovementComponent::ControlledCharacterMove(const FVector& InputVector, float DeltaSeconds)
{
	...
	const FVector input = (InputVector * m_ThrottleNomralization).GetClampedToMaxSize(1.0f);
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	if (CVarAC_DebugReportSplineDecoupling.GetValueOnAnyThread())
	{
		DebugReportSplineCoupling();
	}
#endif
...
void UAC_SplineMovementComponent::HandleInterruption(FVector input, float DeltaSeconds)
{
	...
	if ((m_UrgencyFactor > InterruptionUrgency) && m_SplineState.IsValidSegment())
	{
		...
		float timeToMoveTarget = (m_CurrentMoveTarget - m_Character->GetActorLocation()).Dot(momentumDir) / m_LastRecordedSpeed;
...
float UAC_SplineMovementComponent::CalcMovementReponseTime(const float TimeSinceLastDeflectionChange, const float UrgencyFactor, const float Min, const float Max)
{
	return FMath::Clamp((Min + TimeSinceLastDeflectionChange) * (1.0f - UrgencyFactor), Min, Max);
}

float UAC_SplineMovementComponent::GetCurrentMovementReponseTime(float Min, float Max) const
{
	return CalcMovementReponseTime(m_TimeSinceLastDeflectionChange, m_UrgencyFactor, Min, Max);
}
...
void UAC_SplineMovementComponent::FilloutLookahead(const FVector& Input, float TargetTime, float DeltaT)
{
	...
	// using the characters current (pre step) facing for estimating the rotation target. This is more stable for highly responsive characters
	// but my yield worse results for NPCs and slower/more path following player characters. For these we may want to assess the deviation from the
	// *intended* travel direction along the current spline segment. Although this could add additional work to determine the current spline segment 
	// at least for fast moving characters with very short spline segments where it's possible to overshoot
	const FQuat prevMotionRotation = CharacterOwner->GetActorQuat();
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
void UAC_SplineMovementComponent::DebugReportSplineCoupling()
{		
	if (m_SplineState.IsValidSegment())
	{
		const FVector SplineTestPos = UAC_KBSpline::Sample(m_SplineState);
		const FVector ActorPos = CharacterOwner->GetActorLocation();
		const float DistXY = FVector::Dist2D(ActorPos, SplineTestPos);

		//const float DecoupleTolerance = CVarAC_DebugSplineDecouplingTolerance.GetValueOnAnyThread();
		const float DecoupleTolerance = RailWidth;
		const bool InBounds = DistXY <= DecoupleTolerance;

		const FColor DebugCol = InBounds ? FColor::Green : FColor::Red;
		static const FVector TinySpaceBeamOffset = FVector::UpVector * 100.0f;

		AActor* OwnerActor = GetOwner();

		UE_VLOG(OwnerActor, LogSplineMovement, Verbose, TEXT("%s Distance: %f  Tollerance: %f    Segment: %i   Spline Time: %f"), 
			InBounds ? TEXT("In Bounds") : TEXT("Out of Bounds"),
			DistXY, DecoupleTolerance,
			m_SplineState.CurrentTraversalSegment, m_SplineState.Time
			);
		
		UE_VLOG_LOCATION(OwnerActor, LogSplineMovement, Verbose, ActorPos + TinySpaceBeamOffset, 3, DebugCol, TEXT("Player Position"));
		UE_VLOG_SEGMENT(OwnerActor, LogSplineMovement, Verbose, ActorPos, ActorPos + TinySpaceBeamOffset, DebugCol, TEXT(""));
		UE_VLOG_SEGMENT(OwnerActor, LogSplineMovement, Verbose, SplineTestPos, SplineTestPos + TinySpaceBeamOffset, DebugCol, TEXT("Sampling Position"));
		UE_VLOG_SEGMENT(OwnerActor, LogSplineMovement, Verbose, SplineTestPos + TinySpaceBeamOffset, ActorPos + TinySpaceBeamOffset, DebugCol, TEXT(""));

		if (!InBounds)
		{
			checkf(CVarAC_DebugTrapSplineDecoupling.GetValueOnAnyThread() == false, TEXT("[ValidateSplineCoupling] Trapped execution due to decoupled tolerance"));
		}
	}
}
#endif
...
void UAC_SplineMovementComponent::EvaluateNavigationSpline(float DeltaT)
{
	...
	FVector momentumDir = momentum.GetSafeNormal();
...
bool UAC_SplineMovementComponent::StepSplineStateTargetData(
	const float DeltaT,
...
{
	const FVector& fromPoint = WorkingSplineState.WorkingSet[FKBSplineState::FromPoint].Location;
	FVector currentMoveTarget = InputData.CurrentMoveTarget;
	FVector tangent = FVector::ZeroVector;
	FVector currentOffset = FVector::ZeroVector;
	float currentProjectedMomentum = 0.0f;
	...
	float quantumUpdate = (DeltaT * FMath::Max(InputData.LastRecordedSpeed, 10.0f)) / InputData.CurrentSegLen;
	bool stillTrying = true;

	while (stillTrying && WorkingSplineState.Time <= 1.0f)
	{
		...
		tangent = UAC_KBSpline::ComputeTangent(WorkingSplineState);
		...
		float candidateTime = InputData.SegmentChordDir.Dot(normalizedExpectedStep) / InputData.CurrentSegLen;
		WorkingSplineState.Time = FMath::Max(WorkingSplineState.Time + quantumUpdate, candidateTime);

		currentMoveTarget = UAC_KBSpline::Sample(WorkingSplineState);
		currentOffset = currentMoveTarget - InputData.InitialPos;

		// this part is hacky, should refactor this for a cleaner (and less branchy) flow for ignoring Z offsets
		if (InputData.bForcePlanerOnly)
		{
			currentOffset.Z = 0.0f;
		}
		currentProjectedMomentum = currentOffset.Dot(InputData.MomentumDir);

		float projectedPos = InputData.SegmentChordDir.Dot(InputData.InitialPos - fromPoint);
		float targetProjPos = InputData.SegmentChordDir.Dot(currentMoveTarget - fromPoint);
		...
		if (SubSampleResultFunc)
		{
			SubSampleResultFunc(currentMoveTarget, stillTrying);
		}
	}
	OutputData.Target = currentMoveTarget;
	OutputData.Tangent = tangent;
	OutputData.Offset = currentOffset;
	OutputData.ProjectedMomentum = currentProjectedMomentum;
	return !stillTrying;
}

void UAC_SplineMovementComponent::BuildStepSplineStateInputData(FStepSplineStateInputData& InputDataToBuild) const
{
	FVector momentumDir = Velocity.GetSafeNormal();
	...
	InputDataToBuild.InitialPos = m_Character->GetActorLocation();
	InputDataToBuild.CurrentMoveTarget = m_CurrentMoveTarget;
	InputDataToBuild.MomentumDir = momentumDir;
	InputDataToBuild.SegmentChordDir = m_SegmentChordDir;
	InputDataToBuild.LastRecordedSpeed = m_LastRecordedSpeed;
	InputDataToBuild.CurrentSegLen = m_CurrentSegLen;
	InputDataToBuild.bForcePlanerOnly = bForcePlanerOnly;
}
...
{
	FStepSplineStateInputData InputData;
	FStepSplineStateOutputData OutputData;

	BuildStepSplineStateInputData(InputData);

	TSubSampleResultFunc ResultFunc = nullptr;
	...
	if (CVarAC_SplineDetailedMoveDebug.GetValueOnAnyThread())
	{
		...
		ResultFunc = [OwnerActor](const FVector& SampledPos, const bool StillTryingState)
			{
				UE_VLOG_LOCATION(OwnerActor, LogSplineMovement, Verbose, SampledPos, 3, StillTryingState ? FColor::Black : FColor::Magenta, TEXT("Potential Rabbit"));
			};
	}
#endif

	const bool CompletedSample = StepSplineStateTargetData(DeltaT, InputData, m_SplineState, OutputData, ResultFunc);

	outTarge = OutputData.Target;
	outTangent = OutputData.Tangent;
	outOffset = OutputData.Offset;
	outProjectedMomentum = OutputData.ProjectedMomentum;
...
FRotator UAC_SplineMovementComponent::GetDesiredOrientToMovementRotation(const FRotator& CurrentRotation, const FVector& Velocity, const FRotator& RotationRate)
{
...
FRotator UAC_SplineMovementComponent::ComputeOrientToMovementRotation(const FRotator& CurrentRotation, float DeltaTime, FRotator& DeltaRotation) const
{
	...
	{
		return GetDesiredOrientToMovementRotation(CurrentRotation, Velocity, RotationRate);
	}
...
{
	//DEBUG ONLY, trying to trace NaNs
	auto OriginalVel = Velocity;
...
void UAC_SplineMovementComponent::BuildSplineMovementSampleState(const UKBSplineConfig* SplineConfig, const int PointID, FSplineMovementSampleState& SampleState)
{
	SampleState.State = UAC_KBSpline::PrepareForEvaluation(SplineConfig, PointID);

	UAC_KBSpline::GetChord(SplineConfig, PointID, SampleState.SegmentChordDir);
	
	SampleState.SegmentLength = SampleState.SegmentChordDir.Length();
}
