...
namespace Awful_BeliefNet
{
	...
	public:
		...
		bool TestFlagValue(BBN_Node::NodeStateFlags aValue) const { return (mCurrentState & aValue) == aValue; }
		void SetFlagValue(BBN_Node::NodeStateFlags aValue) { mCurrentState |= aValue; }
		void SetFlagValue(unsigned int aValue) { mCurrentState |= aValue; }
		void ClearFlagValue(BBN_Node::NodeStateFlags aValue) { mCurrentState &= ~aValue; }
		void ClearFlagValue(unsigned int aValue) { mCurrentState &= ~aValue; }
		void FlipFlagValue(BBN_Node::NodeStateFlags aValue) { mCurrentState ^= aValue; }
	...
	private:
		static const NodeKeyType InvalidKey = -1;
